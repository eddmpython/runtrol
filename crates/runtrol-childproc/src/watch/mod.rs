...
mod names;
pub use names::{ChangedNames, DirectoryNames};
...
impl DirectoryChanges {
    ...
    pub fn new(directory: &Path) -> std::io::Result<Self> {
        Self::with_names(directory, true)
    }

    /// Observe names with explicit subtree scope before taking a filesystem snapshot.
    ///
    /// # Errors
    /// Refuses an unavailable, replaced, or unsupported local directory.
    pub fn with_names(directory: &Path, subtree: bool) -> std::io::Result<Self> {
        platform::Changes::new(directory, subtree, false).map(Self)
    }

    /// Observe names and writes before reading provider-owned structural metadata.
    ///
    /// A write only invalidates the caller's observation. It says nothing about model activity.
    ///
    /// # Errors
    /// Refuses an unavailable, replaced, or unsupported local directory.
    pub fn with_writes(directory: &Path, subtree: bool) -> std::io::Result<Self> {
        platform::Changes::new(directory, subtree, true).map(Self)
    }
    ...
    /// Await one invalidation without a timer or a dedicated thread.
    ///
    /// Cancelling the future unregisters its wait before releasing callback memory. The directory
    /// remains observed, so a later waiter still sees changes that arrived while nobody awaited it.
    ///
    /// # Errors
    /// Reports an unavailable notification or replacement of the original directory identity.
...
mod platform {
    ...
    pub(super) fn watch_directory(directory: &Path) -> Option<tokio::sync::mpsc::Receiver<()>> {
        let Ok(owned) = Owned::open(directory, false, false) else {
            return None;
        };
    ...
    impl Owned {
        ...
        fn open(directory: &Path, subtree: bool, writes: bool) -> std::io::Result<Self> {
            ...
            let wide: Vec<u16> = directory
                ...
                .collect();
            let mut flags = FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME;
            if writes {
                flags |= FILE_NOTIFY_CHANGE_LAST_WRITE | FILE_NOTIFY_CHANGE_SIZE;
            }
            let path = wide.as_ptr();
            // SAFETY: wide owns this terminated path, and the flags request directory notifications only.
            let handle = unsafe { FindFirstChangeNotificationW(path, i32::from(subtree), flags) };
    ...
    impl Changes {
        pub(super) fn new(directory: &Path, subtree: bool, writes: bool) -> std::io::Result<Self> {
            require_local_directory(directory)?;
            let identity = directory_identity(directory)?;
            let notification = Owned::open(directory, subtree, writes)?;
        ...
        #[expect(
            unsafe_code,
            reason = "the directory notification remains owned through the borrowed kernel wait"
        )]
        pub(super) async fn wait(&mut self) -> std::io::Result<()> {
            use std::os::windows::io::BorrowedHandle;

            // SAFETY: notification is a live waitable kernel object, exclusively owned by self.
            let handle = unsafe { BorrowedHandle::borrow_raw(self.notification.0) };
            crate::contain::job::wait::signaled(handle)
                .await
                .map_err(std::io::Error::other)?;
            if !self.changed()? {
                return Err(std::io::Error::other("the directory invalidation was lost"));
            }
            Ok(())
        }
    }
    ...
    pub(super) fn require_local_directory(directory: &Path) -> std::io::Result<()> {
    ...
    pub(super) fn directory_identity(directory: &Path) -> std::io::Result<(u64, [u8; 16])> {
    ...
    #[expect(
        unsafe_code,
        reason = "the live file handle and exact FILE_ID_INFO output are borrowed by one identity query"
    )]
    pub(super) fn file_identity(file: &std::fs::File) -> std::io::Result<(u64, [u8; 16])> {
...
mod tests {
    ...
    #[cfg(windows)]
    #[tokio::test]
    async fn write_waits_share_the_kernel_pool_and_survive_cancellation() {
        use std::time::Duration;

        let root = scratch();
        let record = root.join("record.json");
        fs::write(&record, b"{}").expect("initial structural fixture");
        let mut names = DirectoryChanges::new(&root).expect("names only");
        let mut writes = DirectoryChanges::with_writes(&root, false).expect("names and writes");
        assert!(
            tokio::time::timeout(Duration::from_millis(10), writes.wait())
                .await
                .is_err(),
            "an unchanged directory leaves the wait pending"
        );
        fs::write(&record, b"{\"state\":\"busy\"}").expect("change structural fixture");
        tokio::time::timeout(Duration::from_secs(2), writes.wait())
            .await
            .expect("write wake deadline")
            .expect("write notification");
        assert!(
            !names.changed().expect("names remain unchanged"),
            "content writes cannot invalidate a filename-only absence guard"
        );
        drop(writes);
        // One write may produce several notifications. A fresh baseline separates those from cancellation.
        let mut writes = DirectoryChanges::with_writes(&root, false).expect("fresh baseline");
        for _ in 0..8 {
            assert!(
                tokio::time::timeout(Duration::from_millis(1), writes.wait())
                    .await
                    .is_err()
            );
        }
        fs::write(&record, b"{\"state\":\"idle\"}").expect("write after cancelled waits");
        tokio::time::timeout(Duration::from_secs(2), writes.wait())
            .await
            .expect("later wake deadline")
            .expect("later notification");
        drop((writes, names));
        fs::remove_dir_all(root).expect("clean exact fixture");
    }
