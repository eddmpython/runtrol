//! Local administration of what project-scoped Agent Tools left behind: status, listing, inventory, cleanup, and disable.
...
use runtrol_runtime_client::{ClientOptions, LocatorState, ReconnectPolicy, RuntimeLocator};
...
pub async fn run_command(
    ...
    match words.first().map(String::as_str) {
        ...
        Some(other) => Err(AgentToolsError::Mcp(format!(
            "no tools command called {other:?}. try: tools inventory, tools cleanup, tools disable [project], tools status, tools list"
        ))),
        None => Err(AgentToolsError::Mcp(
            "tools needs a command. try: tools inventory, tools cleanup, tools disable [project], tools status, tools list"
                .to_owned(),
        )),
    }
}
