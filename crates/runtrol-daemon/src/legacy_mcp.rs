...
//! inventory of those names and their removal, both through each CLI's own official commands. runtrol never
//! writes a provider's configuration file, never holds the registered state anywhere of its own, and registers
//! nothing. The local half of the same residue, the credential slots, lives in `legacy_slots`.
//!
...
use runtrol_drivers::{McpRegistrar, McpRegistrationState};
use runtrol_ipc::wire::{
    LegacyMcpKind, LegacyMcpLine, LegacyMcpState, LegacyReport, Request, Response,
};
...
const LEGACY_DEADLINE: Duration = Duration::from_secs(15);
...
/// Answer one legacy request: what is left, or what is left after taking ours away.
pub(crate) async fn answer(composed: &Composed, request: &Request) -> Response {
    match request {
        Request::LegacyMcpInventory => Response::LegacyMcpInventory(LegacyReport {
            registrations: legacy_mcp_inventory(composed).await,
            local: crate::legacy_slots::inventory(composed),
        }),
        // Registrations go first, because a registration that outlives its grant is the failure the operator
        // sees: a conversation opening with an MCP server that cannot start.
        Request::LegacyMcpCleanup => Response::LegacyMcpCleanup(LegacyReport {
            registrations: legacy_mcp_cleanup(composed).await,
            local: crate::legacy_slots::cleanup(composed),
        }),
        _ => refuse("legacy preparation does not belong to this request"),
    }
}

/// Whether a request belongs to this module.
pub(crate) const fn is_legacy_mcp(request: &Request) -> bool {
    matches!(
        request,
        Request::LegacyMcpInventory | Request::LegacyMcpCleanup
    )
}
...
async fn cross_consult_inventory(composed: &Composed) -> Vec<LegacyName> {
    ...
    for direction in directions(composed) {
        ...
        let Some(registrar) = from_kind.legacy_mcp.registrar else {
            continue;
        };
...
async fn cross_consult_registration(
    ...
    let Some(serve) = to_kind.legacy_mcp.consult_serve else {
        return registration_without_expected_shape(composed, from_program, registrar, name).await;
    };
    let Ok((to_name, _)) = runtrol_core::locate_named(&direction.to.manifest) else {
        return registration_without_expected_shape(composed, from_program, registrar, name).await;
    };
    exact_registration(composed, from_program, registrar, name, to_name, serve).await
}
...
async fn ask(
    ...
    let args: Vec<String> = words
        ...
        .collect();
    capture(program, &args, LEGACY_DEADLINE, &composed.containment)
        .await
...
mod tests {
    ...
    fn inventory_and_daemon_startup_have_no_legacy_mcp_mutation_path() {
        let source = include_str!("legacy_mcp.rs");
