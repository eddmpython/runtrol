...
| [1-vscodeSurface](1-vscodeSurface/) | 활성 | VS Code 주력 표면, Core 자동 탐색, 단일 hot renderer, 세션별 workspace 및 worktree 결박, Marketplace VSIX | 새 북극성 자체다. 이 표면이 실물로 서지 않으면 뒤의 배포와 폰은 다른 제품을 배포하는 일이 된다. 첫 end-to-end slice, change-only session-index 구독, 실물 Extension Host 기본 성능 ratchet은 구현됐고 Webview 폭주 ratchet, 패키징과 공개 배포가 남았다 |
