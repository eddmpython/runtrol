...
| 폴더 | 상태 | 한 줄 |
|---|---|---|
| [northStarEvidence](northStarEvidence/) | 활성 | 북극성 14 축 각각을 떠받치는 게이트를 실물로 세운다. **게이트 없는 축은 3 점이 상한이다.** |
...
`securityPosture` 는 마일스톤이 아니라 **모든 마일스톤의 골격**이다 (M0 부터 스코프 타입이 함께 간다). `northStarEvidence` 는 각 마일스톤과 동행한다. `어디서나 같은 방법` (macOS·Linux) 은 M2 이후다.
