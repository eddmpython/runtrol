...
| [1-vscodeSurface](1-vscodeSurface/) | 활성 | VS Code 주력 표면, Core 자동 탐색, 단일 hot renderer, 세션별 workspace 및 worktree 결박, Marketplace VSIX | 새 북극성 자체다. 이 표면이 실물로 서지 않으면 뒤의 배포와 폰은 다른 제품을 배포하는 일이 된다. 첫 end-to-end slice, change-only session-index 구독, 실물 Extension Host의 3,000fps Webview 및 8개 hot 세션 전환과 workspace reload ratchet, 공유 이벤트 표현 SSOT, workspace 충돌 선택, Core 원자 worktree 예약, 6개 플랫폼 패키지 확증은 구현됐고 실물 전체 여정, 무중단 갱신 검증, 공개 배포가 남았다 |
