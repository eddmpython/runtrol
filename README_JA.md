...
> - 会話を押すと、その CLI 自身の画面がエディタのタブとして開き、そこに打てば返ってくる。実行中の会話は
>   正確な Runtime PTY か provider の公式 attach のいずれかに付き、`resume` を再実行しない。どちらもなければ、その端末を持つウィンドウを前面に出す。
>   タブは会話の名前を持ち、プロジェクトの色をまとう。
...
- **マルチセッションの費用はセッション数に比例しない。** 15 セッションを日常運用の基準、30 セッションを release gate の負荷とする。論理セッションはさらに存在できるが、hot terminal は最大 8 個である。各会話には一つの生きた owner と、最大一つの Runtime 中央 renderer、bounded ring、screen がある。複数の VS Code 窓と携帯は複製せず共有する。公式 attach は誰かが開くまで renderer も screen memory も割り当てない。選択セッションの固定、即時検索、安定した並び順、workspace 切り替えは 30 セッションでも同じ操作で行う。
...
| 会話を通さない | 6/10 | `egressContract` は実物の loopback socket で正確な送信 allowlist と production Noise IK、IKpsk1 境界を動かす。prompt の標本は relay capture や診断文字列に平文で現れず、transport は disk と log の API を持たない。限定された provider metadata の読み取り境界は [provider architecture](docs/providerArchitecture.md#session-ownership) が定める。実物のスマートフォンと relay を結ぶ live gate がないため、天井は 6 である。 | ユーザーのプロンプトとモデルの応答は、PC とプロバイダーの間、そしてユーザー自身のデバイスの間だけを往復する。runtrol はその本文を保存せず、途中のどのサーバーも読める形でそれを受け取らない。 |
...
| セッション同士が踏まない | 5/10 | 実際の Git metadata と production Core admission が構造化セッションの writer の重複を原子的に拒否する。通常の端末会話は選択した workspace で動き、`courier spawn` worker は Core 所有の linked worktree に結び付く。再開と後処理は[セッション対話契約](docs/sessionDialogue.md)が定める。hosted gate の provider は fixture なので mock 層である。 | どのセッションがどのフォルダで何を変えているかが常に区別でき、二つ目のセッションが同じフォルダに触れそうなときは開始前に警告され、プロバイダーが隔離手段（ワークツリー）を出しているなら開始画面でそのまま使える。 |
| AI 同士が相談し合う | 0/10 | 可視の有効化と双方向の要求・応答を実物 CLI で検証した。現在の動作は[セッション通信契約](docs/sessionDialogue.md)が定める。この軸を採点する有効な CI ゲートはまだない。 | サイドバーで通信を有効にすると、エージェントは別の管理セッションへ明示的に質問し、返答を受け取れる。プロバイダー設定を変更せず、有界メモリだけで本文を転送し、Runtrol は会話のコピーを保持しない。 |
...
- **チャットクライアントではない。** 会話画面は各 CLI が描画する。runtrol は会話の意味を解釈せず、会話記録のコピーを保存しない。画面復元と端末制御のための機械的な VT 処理は[端末契約](docs/terminalSurface.md#host)に従う。
