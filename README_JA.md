...
現在の合計は **19/140、平均 1.4/10** である。ゲートが立っている軸は、いまのところ三つである。
...
| プロバイダー拡張性 | 7/10 | 外部ドライバーは公開 trait だけを使ってコンパイルでき（`providerContract`）、インストール済みの二つの CLI の現在の schema と引数パーサーをバインディング一覧と照合する（`agentSurfaceDrift`）。TOML 一つで汎用 ACP プロバイダーを追加する実環境での証明は残っている。 | 新しい CLI が出たらアダプターを一つ足すだけで、PC 画面もスマホ画面も操作方法もそのまま。ユーザーはプロバイダーが増えたことを一覧が長くなったこととしてだけ知る。 |
