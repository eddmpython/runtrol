...
6. **天井を決めるのは実行回数ではなく、欠けているゲートの種類である。** 一種類しか持たない軸はどれだけ green でも 6 を超えられない。現在 13 軸のうち 12 軸がその状態で、`northStarBoard` が軸ごとにその天井を印字する。
...
| 層 | 何が入るか | どう表示されるか |
|---|---|---|
| **スコア軸** | ユーザーが体感する結果（上の表の 13 個） | 0 から 10、合計 /130 |
...
- **革新性に数字を与えない理由。** 革新とは上の 13 軸そのものである（「複数の AI を一か所で管理する」）。別に点を付ければ同じものを二度数えることになり、どのゲートもその数字を断言できないので規則 3 に触れる。革新が消えたかどうかは kill criteria が判定する。
