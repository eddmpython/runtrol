...
> ステータス: **設計段階。** コードはまだない。以下のスコアがすべて 0 なのは、正直な現状である。
...
現在の合計は **0/130、平均 0.0/10** である。リポジトリにコードがないので、すべての軸が 0 である。
10 点は、実際の環境で完結した道筋が繰り返し検証された状態を指す。
**スコアの根拠は CI で実際に動くゲートである。自動で実行されない経路は、どれほど実装済みに見えても点数に数えない。**

| 北極星 | 現在のスコア | 現状 | 到達すべき状態 |
|---|---:|---|---|
| 一つのセッション一覧 | 0/10 | 未実装。 | プロバイダーが Claude Code でも Codex でも、その次の何かでも、いま自分の PC で生きているセッションが一つの一覧に並び、開始・再開・削除がそこで完結する。 |
| 即座の反応 | 0/10 | 未実装。 | 一覧が待ち時間なく現れ、会話は押した瞬間に開き、長い出力が流れてもスクロールと入力が途切れない。ユーザーが読み込みを意識する瞬間が存在しない。 |
...
| プロバイダー拡張性 | 0/10 | 未実装。アダプター境界もまだない。 | 新しい CLI が出たらアダプターを一つ足すだけで、PC 画面もスマホ画面も操作方法もそのまま。ユーザーはプロバイダーが増えたことを一覧が長くなったこととしてだけ知る。 |
| 会話を通さない | 0/10 | 未実装。 | ユーザーのプロンプトとモデルの応答は、ユーザーの PC とプロバイダーの間だけを往復する。runtrol がその本文を保存したり第三の宛先へ送ったりする経路が存在しない。 |
...
| モデル自動認識 | 0/10 | 未実装。 | いまこのアカウントで実際に使えるモデルがそのまま一覧に出て、新しいモデルが出ても runtrol を直さずに現れる。 |
| セッション同士が踏まない | 0/10 | 未実装。 | 複数のセッションを同時に走らせても互いのファイルを上書きせず、どのセッションが何を変えたかをスマートフォンで区別できる。 |
...
|---:|---|
| **0** | ない。コードもゲートもない。 |
| **3** | 開発機で人が手で一度動くのを見た。自動ゲートはない。 |
| **5** | ゲートは CI で動くが相手が偽物。mock CLI、stub プロバイダー、シミュレートされたスマートフォン。 |
| **8** | 実物の CLI バイナリ、実物のブラウザ、実物のペアリングで端から端までのゲートが CI で動く。ただしプロバイダー一つまたは OS 一つ、幸福な経路のみ。 |
| **10** | 同じゲートがプロバイダー二つ以上、OS 二つ以上（Windows を含む）で動き、障害注入と回帰 ratchet まで含み、実使用で繰り返し検証された。 |

スコアが水増しされないための規則:

1. **実装がどれほど完成して見えても、CI で自動的に動くゲートがなければ上限は 3 点である。** デモ動画、スクリーンショット、「動かしたら動いた」は 3 点。例外なし。
2. 人が手で回す必要のある経路は operator ゲートとして印を付け、**合計から外す**。
3. **偽物相手にしか動かないゲートは上限 5 点である。** mock を実物と呼ぶことがまさに水増しである。
4. ゲートが skip されたり flaky な再試行で通ったりした場合、その軸はその週 5 点を超えない。
5. スコアを上げる PR には**ゲート名と CI 実行へのリンク**を本文に添える。散文はスコアではない。
6. スコアは 0.5 刻みでのみ付ける。8.7 のような数字は精密さではなく自己欺瞞である。
7. **ある軸が下がることを妨げない。** プロバイダーが表面を変えてゲートが red になればスコアは下がる。この表は昨日の自慢ではなく今日の状態である。
