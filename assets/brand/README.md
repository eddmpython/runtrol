...
| 파일 | 크기 | 무엇 |
|---|---|---|
| [symbol-cells.json](symbol-cells.json) | 16x16 | 마크를 셀 격자로. 글자 하나가 픽셀 하나 (`a` 코럴 팔, `i` 잉크 팔, `.` 빈 칸). 작은 크기용 힌트 기하라 획 2, 틈 2, 반지름 3 이 격자에 앉는다. Studio 가 대화를 여는 동안 터미널에 반블록 문자 (`▀` `▄` `█`) 로 이 격자를 그린다. 잉크 팔은 터미널의 기본 글자색이라 테마를 따른다 |
