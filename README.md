...
현재 총점은 **22/140, 평균 1.6/10** 이다. 활성 CI 게이트가 선 축은 셋이고, 로컬 전용 두 축은 manual 층이다.
...
| 공급자 확장성 | 5/10 | hosted CI 는 외부 드라이버 공개 계약 (`providerContract`) 과 외부 TOML 로 ACP fixture 의 전체 턴을 완주하는 경로 (`genericAcpSmoke`) 를 검증한다. 설치된 실물 CLI 의 표면 대조 (`agentSurfaceDrift`) 는 로컬 전용이므로 mock 층을 넘지 않는다. | 새 CLI 가 나오면 어댑터 하나만 추가되고 PC 화면과 폰 화면과 조작 방법은 그대로다. 사용자는 공급자가 늘어난 것을 목록이 길어진 것으로만 안다. |
...
| 모델 자동 인식 | 3/10 | `modelDetectionSmoke` 는 로컬 preflight 에서 설치된 실물 CLI 와 계정별 모델을 검사하지만 hosted CI 에서 자동 실행되지 않는다. 따라서 manual 층을 넘지 않는다. | 지금 이 계정으로 쓸 수 있는 모델이 목록에 그대로 뜨고, 새 모델이 나와도 runtrol 을 고치지 않아도 뜬다. |
