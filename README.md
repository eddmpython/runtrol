...
[독립 실행 CLI를 설치](docs/runtimeOperations.md#install-and-start)한 뒤 새 터미널에서 Runtime 상태를 확인한다.
...
>   resume하지 않고 데몬의 정확한 PTY 나 provider 공식 attach 중 증명된 경로에 붙고, 둘 다 없으면 그 터미널을 가진 창을 앞으로 부른다. 탭은
...
| 대화 무통과 | 6/10 | 실물 루프백 소켓의 정확한 송신 허용 목록과 production Noise IK 및 IKpsk1 경계가 돈다 (`egressContract`). 프롬프트 표본은 릴레이 캡처와 진단 문자열에 평문으로 나타나지 않고, transport 는 디스크와 로그 API 를 갖지 않는다. 제한된 공급자 메타데이터 읽기의 경계는 [공급자 아키텍처](docs/providerArchitecture.md#session-ownership)가 소유한다. 실물 폰과 릴레이를 잇는 live 게이트가 없어 천장이 6 이다. | 사용자의 프롬프트와 모델의 응답은 PC 와 공급자 사이, 그리고 사용자 자신의 기기 사이에서만 오간다. runtrol 은 본문을 저장하지 않고, 중간의 어떤 서버도 그것을 읽을 수 있는 형태로 받지 않는다. |
...
| 세션끼리 안 밟기 | 5/10 | 실제 Git metadata와 production Core admission이 구조화 세션의 겹치는 writer를 원자적으로 거부한다. 일반 터미널 대화는 선택한 workspace에서 실행되고, `courier spawn` 작업자는 Core 소유 linked worktree에 결박된다. 작업자의 재개와 정리는 [세션 대화 계약](docs/sessionDialogue.md)이 소유한다. 활성 hosted 게이트의 provider는 fixture이므로 점수는 mock 층에 머문다. | 어느 세션이 어느 폴더에서 무엇을 고치는지 항상 구분되고, 두 세션이 같은 폴더를 만지게 되면 시작 전에 경고받으며, 공급자가 격리 수단 (워크트리) 을 내주면 시작 화면에서 그대로 쓴다. |
| AI 끼리 서로 자문 | 0/10 | 관리 세션의 대화 활성화와 양방향 요청·응답을 실물 CLI로 검증했다. 현재 동작은 [세션 대화 계약](docs/sessionDialogue.md)이 소유한다. 이 축을 점수로 인정하는 활성 CI 게이트는 아직 없다. | 사이드바에서 대화를 켜면 에이전트가 다른 관리 세션에 명시적으로 묻고 답을 받는다. 공급자 설정을 바꾸지 않고, 본문은 제한된 메모리에서만 전달하며 Runtrol에 대화 사본을 남기지 않는다. |
...
- **채팅 클라이언트가 아니다.** 대화 화면은 각 CLI가 만든다. runtrol은 대화의 의미를 해석하거나 대화 사본을 저장하지 않는다. 화면 복원과 터미널 제어를 위한 기계적 VT 처리는 [터미널 계약](docs/terminalSurface.md#host)을 따른다.
