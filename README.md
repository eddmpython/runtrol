...
| 폰에서 내 PC 세션 잇기 | 0/10 | 릴레이 기반 PWA, Rust 및 WebCrypto 상호운용, 내용 없는 Web Push가 구현됐다. 실물 휴대폰과 실물 CLI를 잇는 활성 게이트가 없어 점수는 아직 0이다. | 폰을 PC 에 한 번 붙여 두면, 자리를 떠난 뒤에도 그 PC 에서 돌고 있는 세션에 폰에서 새 지시를 넣고 출력을 실시간으로 본다. 공급자 계정의 등급이나 인증 방식이 이 경험을 막지 않는다. |
...
| 폰에서 승인 | 0/10 | 위험도별 권한과 정확한 승인 대상을 표시하는 PWA 표면은 구현됐다. 실물 공급자 승인 종단 게이트가 없어 점수는 아직 0이다. | 에이전트가 위험한 작업 앞에서 멈추면 폰에 뜨고, 폰에서 허용하거나 거부하면 PC 의 세션이 즉시 이어진다. |
