...
현재 총점은 **24/140, 평균 1.7/10** 이다. 게이트가 선 축은 넷이다.
...
| 공급자 확장성 | 7/10 | 외부 드라이버가 공개 trait 만으로 컴파일되고 (`providerContract`), 설치된 두 CLI 의 현재 스키마와 인자 파서를 바인딩 목록과 대조한다 (`agentSurfaceDrift`). 외부 TOML 하나가 별도 ACP 프로세스를 발견해 전체 턴을 완주하는 경로도 검증한다 (`genericAcpSmoke`). 이 마지막 상대는 fixture 라 실물 공급자 가산에는 세지 않는다. | 새 CLI 가 나오면 어댑터 하나만 추가되고 PC 화면과 폰 화면과 조작 방법은 그대로다. 사용자는 공급자가 늘어난 것을 목록이 길어진 것으로만 안다. |
