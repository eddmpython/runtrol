...
현재 총점은 **19/140, 평균 1.4/10** 이다. 게이트가 선 축은 셋이다.
10 점은 실제 환경에서 완결 여정이 반복 검증된 상태다.
**점수의 근거는 CI 에서 실제로 도는 게이트다. 자동으로 실행되지 않는 경로는 구현돼 있어도 점수로 세지 않는다.**

| 북극성 | 현재 점수 | 현 상태 | 도달할 상태 |
|---|---:|---|---|
| 하나의 세션 목록 | 6/10 | 실물 CLI 두 개의 세션이 한 목록에 뜨고, 시작·닫기가 돌고, 목록이 재개에 필요한 이름을 싣는다 (`sessionLifecycleSmoke`, 토큰 0). 성공한 재개는 아직 게이트 밖이다 (턴이 있는 대화가 필요하다). 정적 계약 게이트가 없어 천장이 6 이다. | 공급자가 Claude Code 든 Codex 든 그 다음 무엇이든, 지금 내 PC 에 살아 있는 세션이 한 목록에 뜨고 거기서 시작, 재개, 삭제가 끝난다. |
...
| 공급자 확장성 | 7/10 | 외부 드라이버가 공개 trait 만으로 컴파일되고 (`providerContract`), 설치된 두 CLI 의 현재 스키마와 인자 파서를 바인딩 목록과 대조한다 (`agentSurfaceDrift`). 범용 ACP 드라이버를 TOML 하나로 붙이는 실물 검증은 남았다. | 새 CLI 가 나오면 어댑터 하나만 추가되고 PC 화면과 폰 화면과 조작 방법은 그대로다. 사용자는 공급자가 늘어난 것을 목록이 길어진 것으로만 안다. |
| 대화 무통과 | 0/10 | 미구현. | 사용자의 프롬프트와 모델의 응답은 PC 와 공급자 사이, 그리고 사용자 자신의 기기 사이에서만 오간다. runtrol 은 본문을 저장하지 않고, 중간의 어떤 서버도 그것을 읽을 수 있는 형태로 받지 않는다. |
...
| 모델 자동 인식 | 6/10 | 실물 CLI 두 개를 매 preflight 에 묻는다. 열거 가능한 CLI 는 현재 계정의 모델을 내고, 열거하지 못하는 CLI 는 매니페스트의 안정된 별칭과 한계를 그대로 밝힌다 (`modelDetectionSmoke`, 토큰 0). 발견된 모델 식별자가 드라이버 밖 소스에 리터럴로 박히지 않았는지도 검사한다. 별도 정적 계약 게이트가 없어 천장이 6 이다. | 지금 이 계정으로 쓸 수 있는 모델이 목록에 그대로 뜨고, 새 모델이 나와도 runtrol 을 고치지 않아도 뜬다. |
| 세션끼리 안 밟기 | 0/10 | 미구현. | 어느 세션이 어느 폴더에서 무엇을 고치는지 항상 구분되고, 두 세션이 같은 폴더를 만지게 되면 시작 전에 경고받으며, 공급자가 격리 수단 (워크트리) 을 내주면 시작 화면에서 그대로 쓴다. |
| AI 끼리 서로 자문 | 0/10 | 미구현. | 토글 하나로 두 CLI 가 서로를 공식 표면 (MCP) 으로 등록해, 한 AI 가 턴 중에 다른 AI 의 의견을 직접 받아온다. 배선은 CLI 자신의 공식 명령으로만 만들고 (설정 파일을 직접 쓰지 않는다), 대화 본문은 여전히 runtrol 을 지나지 않는다. 사용자가 MCP 라는 개념을 몰라도 된다. |
...
점수는 사람이 고르는 등급이 아니라 **기반 층 + 가산, 상한으로 깎기**로 계산된다. 정본은
...
| `manual` | 3 | 사람이 손으로 한 번 봤다. 러너에 등록된 게이트가 없다. 데모 영상, 스크린샷, "돌려봤더니 되더라" 가 전부 여기다 |
...
**상한.** 위에서 무엇을 주장했든 내리누른다.

| 상한 | 결과 | 성립 조건 |
|---|---:|---|
| `skipped` | 5 | 마지막 실행에서 게이트가 skip 됐다. skip 은 통과가 아니다 |
| `flaky` | 5 | 재시도로 통과했다 |
| `unregistered` | 0 | 게이트 파일은 있는데 어느 러너도 부르지 않는다. 게이트가 없는 것과 같다 |
