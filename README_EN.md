...
The current total is **71/140, average 5.1/10**. Thirteen axes have active CI gates.
...
| Sessions do not trample each other | 5/10 | Real Git metadata and production Core admission atomically reject overlapping structured-session writers. Ordinary terminal conversations run in the selected workspace; `courier spawn` workers bind to Core-owned linked worktrees. [Session dialogue](docs/sessionDialogue.md) owns worker resume and cleanup. The hosted gate uses a fixture provider, so this remains the mock tier. | Which session is touching which folder is always distinguishable, starting a second session into the same folder warns before it happens, and when a provider offers isolation (worktrees) it is available right on the start screen. |
| Agents consult each other | 0/10 | Visible activation and bidirectional requests and replies have been verified with real CLIs. The [session dialogue contract](docs/sessionDialogue.md) owns current behavior. No active CI gate yet scores this axis. | Enabling dialogue in the sidebar lets an agent explicitly ask another managed session and receive its reply. Provider configuration stays untouched; bounded memory transports the body and Runtrol retains no conversation copy. |
