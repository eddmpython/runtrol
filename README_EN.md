...
| Current without asking | 5/10 | `vscodeUpgradeRollback` atomically replaces the packaged Core at one stable path and verifies on all three operating-system CI runners that the original daemon, provider PID, session, and workspace survive extension upgrade and rollback. Provider updating is still absent, so the evidence is mock tier. | The app and the installed agent CLIs stay current on their own, and if an update breaks a session it has already rolled back before the user touches anything. There is no moment where the user thinks about versions. |
