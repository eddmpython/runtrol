...
| Provider extensibility | 5/10 | Hosted CI checks the public outside-driver contract, a generic ACP fixture on three operating systems, and two turns plus native load through an independently distributed ACP implementation. Its model endpoint is a local mock. Scheduled CI checks only unauthenticated schema-method and parser-flag probes against current real CLIs, not the complete event and control frame surface. | When a new CLI appears, one adapter is added and the PC screen, the phone screen, and the controls stay the same. The user notices a new provider only as a longer list. |
