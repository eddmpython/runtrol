...
> - 点开会话，那个 CLI 自己的界面会作为编辑器标签打开，直接输入就有回应。已经运行的会话会连接到准确的
>   Runtime PTY 或 provider 官方 attach，不会再次执行 `resume`；两者都没有时，把拥有该终端的窗口带到前台。标签带着会话名，用项目的颜色。
...
- **多会话成本不随会话数增长。** 15 个会话是日常使用基线，30 个会话是发布门槛负载。逻辑会话可以更多，但 hot terminal 最多 8 个。每个对话只有一个存活 owner，以及最多一个 Runtime 中央 renderer、bounded ring 与 screen。多个 VS Code 窗口和手机共享它们而不复制。官方 attach 在有人打开前不分配 renderer 或 screen 内存。固定当前选择、即时搜索、稳定排序和 workspace 切换在 30 个会话时仍使用相同操作。
...
当前总分为 **71/140，平均 5.1/10**。十三个轴由启用的 CI 门禁支撑。
...
| 对话不经过 | 6/10 | `egressContract` 在真实回环套接字上运行精确的出站白名单和 production Noise IK、IKpsk1 边界。提示词样本不会以明文出现在中继捕获或诊断信息中，transport 没有磁盘或日志 API。受限的供应商元数据读取边界由 [provider architecture](docs/providerArchitecture.md#session-ownership) 定义。尚无连接真实手机与中继的 live 门禁，所以天花板是 6。 | 用户的提示词与模型的回复只在电脑与供应商之间、以及用户自己的设备之间往返。runtrol 不保存其内容，中间的任何服务器都不会以可读的形式收到它。 |
...
| 到哪都一样 | 8/10 | 启用的 hosted CI 会把精确的原生 VSIX 安装到干净的 VS Code 中，在没有手动 Core 路径的情况下发现内置 Core，打开 Runtrol，并通过公开命令打开再关闭 `New chat` 编写标签页。同一门禁运行于 Windows、macOS、Linux，发布矩阵还会在 x64 与 ARM64 的六个目标上重复该旅程。这里只宣称静态契约、真实旅程与多操作系统证据。 | 在 Windows、macOS、Linux 上安装方式与操作方式相同。Windows 用户无需知道 WSL 或 tmux 是什么。 |
...
| 会话之间不互踩 | 5/10 | 真实 Git metadata 与 production Core admission 原子拒绝结构化会话中重叠的 writer。普通终端会话在选定的 workspace 中运行，`courier spawn` worker 则绑定 Core 管理的 linked worktree。恢复与清理由[会话通信契约](docs/sessionDialogue.md)定义。hosted gate 使用 fixture provider，因此仍属于 mock 层。 | 哪个会话在哪个文件夹改了什么始终可以区分；第二个会话将要触碰同一文件夹时，在开始前就会收到警告；供应商提供隔离手段（worktree）时，可直接在开始界面使用。 |
...
- **不是聊天客户端。** 每个 CLI 渲染自己的对话界面。runtrol 不解释对话含义，也不保存对话记录副本。用于屏幕恢复和终端控制的机械性 VT 处理遵循[终端契约](docs/terminalSurface.md#host)。
