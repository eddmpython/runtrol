...
6. **决定天花板的是缺失的门禁种类，不是运行次数。** 只有一种门禁的轴，无论跑得多绿都过不了 6。目前 13 个轴里有 12 个处在这个状态，`northStarBoard` 会在每个轴旁边印出它的天花板。
...
| 层 | 里面放什么 | 如何呈现 |
|---|---|---|
| **计分轴** | 用户能感受到的结果（上表的 13 个） | 0 到 10，合计 /130 |
...
- **为什么创新性不给数字。** 创新就是上面这 13 个轴本身（"在一个地方管理多个 AI"）。单独给分等于把同一件事数两遍，而且没有任何门禁能断言那个数字，正好撞上规则 3。创新是否还成立，由 kill criteria 判定。
