...
当前总分为 **19/140，平均 1.4/10**。目前有三个轴背后站着门禁。
...
| 供应商可扩展性 | 7/10 | 外部驱动只依赖公开 trait 即可编译（`providerContract`），并把两个已安装 CLI 的当前 schema 与参数解析器同绑定清单对照（`agentSurfaceDrift`）。仍需用真实环境证明仅靠一个 TOML 文件即可接入通用 ACP 供应商。 | 出现新的 CLI 时只需增加一个适配器，电脑界面、手机界面与操作方式保持不变。用户只会感到列表变长了。 |
