...
const COPY = {
  en: {
    ...
    heroLede: "runtrol supervises the coding-agent CLIs already installed on your machine. Every project, 30 conversations at once, who is running, and how much quota is left, in one sidebar, with no extra window and no second copy of your chats.",
  ...
  ko: {
    ...
    heroLede: "runtrol은 컴퓨터에 이미 설치된 코딩 에이전트 CLI를 감독합니다. 모든 프로젝트, 한 번에 30개 대화, 지금 누가 돌고 있는지, 사용량이 얼마나 남았는지를 사이드바 한 화면에서 봅니다. 별도 창도, 대화 사본도 없습니다.",
  ...
  zh: {
    ...
    heroLede: "runtrol 监督你电脑上已经安装的编码代理 CLI。每个项目、同时 30 段对话、谁在运行、配额还剩多少，都在一个侧边栏里，没有额外窗口，也没有对话副本。",
  ...
  ja: {
    ...
    heroLede: "runtrol はこの PC にすでに入っているコーディングエージェント CLI を監督します。すべてのプロジェクト、同時に 30 の会話、いま誰が動いているか、残り使用量を一つのサイドバーで。別ウィンドウも会話のコピーもありません。",
