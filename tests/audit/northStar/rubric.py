...
This module takes the rungs apart into the facts they were made of::
    ...
    score = min(tier + additives, MAXIMUM)

Exactly one tier applies and it is a ceiling, not an opening bid. Additives attach only at the top
tier, because breadth measured against a fake counterpart is breadth over nothing.
...
TIERS: dict[str, tuple[float, str]] = {
    ...
    "manual": (3.0, "someone ran it by hand. no gate is active in hosted CI"),
...
def scoreOf(tier: str, additives: tuple[str, ...], kinds: frozenset[str]) -> Score:
    ...
    return Score(value=earned, ceiling=ceilingFor(kinds), heldBy=reasonFor(tier, additives))
...
def reasonFor(tier: str, additives: tuple[str, ...]) -> str:
...
def problemsFor(tier: str, additives: tuple[str, ...], kinds: frozenset[str]) -> list[str]:
