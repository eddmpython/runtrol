...
Six crates here have a real reason to opt out, each found by trying to do it the other way:
...
- `runtrol-ipc` installs and verifies current-user Windows named-pipe and file ACLs.
- `runtrol-runtime-client` validates the Windows locator owner and protected DACL before trusting it.
- `runtrol-security` binds approved Windows project roots to stable filesystem identity.
In these platform crates, `forbid` cannot be relaxed from inside a crate that inherits it, and cargo hard-errors
