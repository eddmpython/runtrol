...
| `vscodeExtension` | The primary VS Code surface has no shipped Node runtime dependencies, polling loop, browser persistence, or filesystem write API. It keeps one selected watch, bounds IPC queues and rendered conversation nodes, and checks segment-aware workspace collisions before starting another hot writer. Equality, ancestry, sibling prefixes, Windows casing, detached sessions, explicit override, type checking, tests, and bundle size are gated. |
