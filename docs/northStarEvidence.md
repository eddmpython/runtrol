...
| `vscodePackage` | The extension release SSOT names one publishable SemVer and six native targets. Fault injection rejects missing or extra files, source and tooling leaks, wrong target or version metadata, license drift, stub binaries, unsafe paths, and Core byte drift. Release jobs resolve Core paths from the repository root and assemble in isolated temporary staging, so a running development Core is untouched. Each job then inspects the built VSIX, installs it into a clean stable VS Code profile with no configured Core path, activates through the bundled Core, and requires exact process cleanup. |
