...
| `egressContract` | Real loopback connections enforce the production destination allowlist. Production Noise session and pairing paths authenticate peer and link context, reject tampering and incorrect keys, and preserve framing and rekey round trips. The sampled prompt is absent as plaintext from relay capture and diagnostic formatting. Static checks reject listed persistence and logging APIs in transport and specific transcript-path literals in driver and store sources. The latter is not proof that provider-owned metadata discovery is absent; [session ownership](providerArchitecture.md#session-ownership) defines that boundary. |
...
| `idleFootprintRatchet` | Hosted Windows, macOS, and Linux run the real debug daemon against the idle RSS ceiling in [`memoryBudget.rs`](../tests/audit/memoryBudget.rs) and the CPU and sampling ceilings in [`idleFootprintRatchet.py`](../tests/audit/idleFootprintRatchet.py), including its quiet-settling procedure. A window with no locally launched CLI can still subscribe to active external providers, so its observation cost is not automatically an idle sample. This gate does not claim release-build live cost or GUI cost. |
| `crossPlatformContract` | The gate reuses the target catalogue consumed by `vscodePackage` to verify each native executable, platform family, and hosted runner. Both the ordinary operating-system jobs and complete package matrix must run first-use verification on the exact archive. The same user path fixes Studio's public new-conversation command, platform shortcut, empty `runtrol.corePath` default, and installed-build draft identity. |
| `crossPlatformMatrix` | A clean real VS Code profile installs the exact native VSIX, discovers and materializes its bundled Runtime without a manual path, opens Runtrol, opens and closes the exact `New chat` draft through the public `runtrol.startSession` command, and repeats for every entry in [`release-targets.json`](../extensions/runtrol-vscode/release-targets.json). It starts no prompt or provider process and does not claim model behavior, multiple-provider behavior, or recovery. |
...
| `vscodeExtension` | Studio contributes one provider-neutral `runtrol.sidebar` webview containing project rows, conversation rows, first-run actions, and compact usage gauges in one page. The gate rejects extra view headers, a native-tree regression, hardcoded provider branches, hidden coverage warnings, unavailable keyboard actions, invented usage percentages, an absent theme color, or a private terminal path. Source boundaries permit only bounded operational metadata and reviewed managed-Runtime installation writes, with no conversation-capable filesystem writer, transcript persistence, polling loop, or shipped Node runtime dependency. Type checking, tests, bundle size, exact command visibility, native icons, and segment-aware workspace collisions are gated. |
| `vscodePackage` | The extension release SSOT names one publishable SemVer and the exact native executable, family, and hosted runner for every release target. Fault injection rejects a missing target, a wrong architecture runner, missing or extra files, source and tooling leaks, wrong target or version metadata, license drift, stub binaries, unsafe paths, Core byte drift, a floating publisher CLI, unsupported Marketplace authentication, an unpinned workflow action, a missing automatic version trigger, and incomplete installation or support metadata. Release jobs resolve Core paths from the repository root and assemble in isolated temporary staging, so a running development Core is untouched. The exact gated main release commit freezes one governed annotated-tag object and peeled release SHA, then creates a tag-bound draft Release. Valid exact-SHA staged assets survive retries, while missing or invalid targets alone are rebuilt and reconciled. The complete staged set is audited before Marketplace publication. Public install journeys must pass on the full target matrix, and remote draft bytes and live Marketplace digests must match the locally audited archives before the GitHub Release is exposed. Final verification binds its message, state, assets, commit labels, bytes, tag object, and current-main ancestry to the exact release. The read-only `publishExisting` route validates the existing public Release and bytes without imposing the newer asset-label contract. |
...
| `runtimeDistribution` | The complete native standalone Runtime ZIP catalogue derives from the shared target manifest. Each archive contains one real headless binary and an exact allowlist, reproduces checksums, carries protocol and rollback metadata, refuses running-locator uninstall, preserves provider-owned state, emits a machine-verifiable uninstall result, and requires pinned keyless release attestations. The release manifest fails if any Runtime or SDK artifact is missing. |
...
| `vscodeHostPerformance` | A real isolated VS Code Extension Host runs the production bundle against a tracked product Core. Isolated cold trials feed the field-wise fastest complete result into the shared JSON ratchet, while the exact managed and hot counts must hold in every trial. The ratchet covers ready activation from one exact initial Runtime inventory, Runtrol navigation and editor conversation opening, refresh p95, RSS growth, cold native resume, hot-session switching, reload selection restoration, and live workspace following. The gate runs on hosted Windows, macOS, and Linux with exact session and process cleanup. [`performance-budget.json`](../extensions/runtrol-vscode/performance-budget.json) owns its release load and ceilings. |
| `vscodeMultiWindowTerminal` | Two simultaneous real VS Code Extension Hosts load the production extension and attach editor terminal tabs to the exact same Runtime generation, terminal id, terminal generation, and provider TUI process. Input from the first window is observed in both windows. The first window then exits, the exact provider PID generation remains alive, the second window acquires the writer path and receives its own provider output, and its stop action ends that exact provider generation. Independent raw series prove first-use dispatch and warm sender, fan-out, and handoff latency against [`performance-budget.json`](../extensions/runtrol-vscode/performance-budget.json). A create-new PID marker makes a second fixture owner fail closed, summaries are recomputed from samples, and cleanup requires zero task-owned survivors. |
...
| `liveMemoryBudget` | The real debug daemon and external ACP fixture hold the complete hot-idle admission set, then exercise one hot session with multiple live watchers, complete admitted delivery, explicit lag for an oversized live payload, bounded allocator settling, and repeated rejection without retained growth. [`liveMemoryBudget.py`](../tests/audit/liveMemoryBudget.py) owns the executable payload, RSS, residual, sampling, and cardinality values; this row owns only the proof boundary. |
