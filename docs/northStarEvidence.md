...
| `vscodeExtension` | The primary VS Code surface has no shipped Node runtime dependencies, polling loop, browser persistence, or conversation-capable filesystem writer. Its only disk mutations are one bounded selected-session scalar and the reviewed managed-Core installation primitive. It keeps one selected watch and one editor conversation panel, bounds IPC queues and rendered conversation nodes, derives readable fallback names without conversation content, preserves bounded operator names, and checks segment-aware workspace collisions before starting another hot writer. Equality, ancestry, sibling prefixes, Windows casing, detached sessions, explicit override, type checking, tests, and bundle size are gated. |
| `vscodePackage` | The extension release SSOT names one publishable SemVer and six native targets. Fault injection rejects missing or extra files, source and tooling leaks, wrong target or version metadata, license drift, stub binaries, unsafe paths, Core byte drift, a floating publisher CLI, unavailable or secret Marketplace publishing paths, unpinned workflow actions, and missing English installation or support metadata. Release jobs resolve Core paths from the repository root and assemble in isolated temporary staging, so a running development Core is untouched. Each job then inspects the built VSIX, installs it into a clean stable VS Code profile with no configured Core path, activates through the bundled Core, and requires exact process cleanup. The workflow creates a tagged GitHub Release only after all six packages pass. Release 0.1.0 is public for all six native targets, and stable VS Code 1.132.1 downloaded and activated the public `win32-x64` package with no configured Core path. Marketplace upload used the pinned client with a short-lived Marketplace-only credential that was removed locally after publication; no publishing secret is stored in the repository or workflow. |
| `versionSsot` | `[workspace.package].version` is the only publishable release version in source. Every Cargo member inherits it, and VS Code packaging derives it into temporary staging while checked-in npm manifests keep the neutral `0.0.0` placeholder. Fault injection rejects a hardcoded member, extension, lockfile, or disconnected derivation module. |
