...
| `vscodeExtension` | The primary VS Code surface has no shipped Node runtime dependencies, polling loop, browser persistence, or conversation-capable filesystem writer. Its only disk mutations are one bounded selected-session scalar and the reviewed managed-Core installation primitive. It keeps one selected watch, bounds IPC queues and rendered conversation nodes, and checks segment-aware workspace collisions before starting another hot writer. Equality, ancestry, sibling prefixes, Windows casing, detached sessions, explicit override, type checking, tests, and bundle size are gated. |
...
| `vscodeHostPerformance` | A real isolated VS Code Extension Host runs the production bundle against a tracked product Core. Three isolated cold trials feed one median into a shared JSON ratchet, so one hosted-runner scheduling outlier cannot relax or falsely trip the contract. The ratchet caps ready activation, contributed-view opening, 40-refresh p95, RSS growth, and real contributed-Webview animation, input, scroll, and queue growth while 3,000 raw frames per second cross the extension boundary with zero dropped frames in every trial. Each trial starts 30 external-manifest ACP sessions in independent workspaces while the Core keeps at most eight provider children hot, resumes one cold row through the provider-native surface within 1,500 ms, requires every hot switch to receive a new Core watch acknowledgement and Webview paint, and restarts VS Code in the selected workspace with the same profile to prove exact selection restoration. Animation records the runner's unloaded native cadence first, then enforces both a 40 ms absolute frame ceiling and an 8 ms p95 load-overrun ceiling. Switch p95 is capped at 100 ms and complete reload restoration at 1,500 ms. The gate runs on hosted Windows, macOS, and Linux with exact session and process cleanup. |
...
| `desktopEventCoverage` | All 19 names from `EventBody::wire_name` map to one bounded presentation contract in `assets/event-presentation.json`. Desktop and VS Code consume the same kind, side, and localization-key SSOT, while keeping localized human text and unknown-event fallback in each surface. Fault injection rejects missing, stale, malformed, opaque-content, and surface-local event maps. |
