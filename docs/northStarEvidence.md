...
| `crossPlatformMatrix` | 정확한 네이티브 VSIX를 깨끗한 실물 VS Code 프로필에 설치하고, 수동 Core 경로 없이 번들 Core를 발견·복사한 뒤 Runtrol을 연다. 같은 공개 `runtrol.startSession` 명령으로 `Runtrol: New chat` 작성 탭을 열고 그 정확한 draft를 닫는다. 일반 hosted CI의 Windows·macOS·Linux에서 동일 게이트가 돌고, 릴리스 행렬은 x64와 ARM64 6개 대상에서 반복한다. prompt나 provider process는 시작하지 않으며 model 동작, 다중 provider, 장애 복원은 주장하지 않는다 |
...
| `vscodeExtension` | The primary VS Code surface has no shipped Node runtime dependencies, polling loop, browser persistence, or conversation-capable filesystem writer. Its disk mutations are limited to one bounded selected-session scalar, the reviewed managed-Core installation primitive, one cross-window project-lease directory, and the exact Receipt Landing transaction. Landing binds source bytes to Receipt path, size and SHA-256 evidence, preflights all allocations, rejects links and dirty text, notebook or custom-editor tabs, atomically replaces targets after final CAS, and verifies guarded rollback. Ordinary Landing combines every non-overlapping passing Receipt; Fleet winner Landing binds exactly one selected passing Task through review, apply, Gates, Core completion, and durable terminal Task and Receipt evidence, and never mixes another candidate. Core compares Receipt evidence before and after Gates. Interrupted-Mission recovery freezes Mission, policy, instruction, provider, workspace, base, and Task-state authority before warning about duplicate external effects, rechecks it after confirmation, accepts cancellation without mutation, and shares one provider-neutral wave runner with Fleet. A second stop between Task reopen and scheduler resume remains re-enterable, while an ambiguous Send is not repeated. Core-owned Mission scheduling contributes schedule and exact-cancel actions, freezes Mission, policy, Task instruction, selector, workspace, provider, due-time, and replacement-CAS authority, rechecks it after confirmation, and contains no Extension timer. Static mutations remove that authority, replacement ID, activation, and command reachability or inject a timer, and each makes the gate red. The surface keeps one selected watch and one editor conversation panel, bounds IPC queues and rendered conversation nodes, derives readable fallback names without conversation content, preserves bounded operator names, and checks segment-aware workspace collisions before starting another hot writer. Equality, ancestry, sibling prefixes, Windows casing, detached sessions, explicit override, type checking, tests, and bundle size are gated. |
| `vscodePackage` | The extension release SSOT names one publishable SemVer and the exact native executable, family, and hosted runner for six targets. Fault injection rejects a missing target, a wrong architecture runner, missing or extra files, source and tooling leaks, wrong target or version metadata, license drift, stub binaries, unsafe paths, Core byte drift, a floating publisher CLI, unsupported Marketplace authentication, an unpinned workflow action, a missing automatic version trigger, and incomplete installation or support metadata. The static workflow contract requires the unconditional `crossPlatformMatrix` first-run step inside the six-target package job and binds it to the exact matrix archive. Release jobs resolve Core paths from the repository root and assemble in isolated temporary staging, so a running development Core is untouched. Each job inspects the built VSIX, installs it into a clean stable VS Code profile with no configured Core path, activates through the bundled Core, and requires exact process cleanup. A patch increment on `main` publishes through the Actions `VSCE_PAT` secret, verifies all six public package digests, and then installs and activates the exact Marketplace version on all six native runners. Release 0.1.11 is tagged only after those public journeys pass. |
...
| `runtimePublicBoundary` | Publishable Rust and TypeScript Runtime clients can import only the provider-neutral public protocol. Runtrol Studio uses the public TypeScript package for ordinary session behavior, private administration cannot enter the public dispatcher, and generated schema bytes match their Rust source. |
...
| `vscodeHostPerformance` | A real isolated VS Code Extension Host runs the production bundle against a tracked product Core. Three isolated cold trials feed one median into a shared JSON ratchet, so one hosted-runner scheduling outlier cannot relax or falsely trip the contract. The ratchet caps ready activation from one exact initial Runtime inventory, Runtrol navigation and editor conversation opening confirmed by both panel and active-tab state, 40-refresh p95, RSS growth, and real Webview animation, input, scroll, and queue growth while 3,000 raw frames per second cross the extension boundary with zero dropped frames in every trial. It also covers hiding the conversation behind a text editor and restoring the same editor tab before load begins. Each trial starts 30 external-manifest ACP sessions in independent workspaces while the Core keeps at most eight provider children hot, resumes one cold row through the provider-native surface within 1,500 ms, requires every hot switch to receive a new Core watch acknowledgement and Webview paint, and restarts VS Code in the selected workspace with the same profile to prove exact selection restoration. Animation records the runner's unloaded native cadence first, then enforces both a 40 ms absolute frame ceiling and an 8 ms p95 load-overrun ceiling. Switch p95 is capped at 125 ms and complete reload restoration at 1,750 ms. The gate runs on hosted Windows, macOS, and Linux with exact session and process cleanup. |
