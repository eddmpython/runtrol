...
| `vscodeHostPerformance` | A real isolated VS Code Extension Host runs the production bundle against a tracked product Core. One shared JSON ratchet caps ready activation, contributed-view opening, 40-refresh p95, and RSS growth. The gate runs on hosted Windows, macOS, and Linux with exact daemon and temporary-profile cleanup. Webview burst rendering is not part of this gate. |
