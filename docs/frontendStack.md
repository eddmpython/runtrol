...
The landing uses one source object for the repository and latest-release endpoint. A manual download becomes active
only when the GitHub API returns a `.vsix` asset, and `release-assets.mjs` matches the runtime operating system and
architecture to the catalogue in
[`release-targets.json`](../extensions/runtrol-vscode/release-targets.json). An unknown target opens the tagged release
instead of guessing. The page never hardcodes a release version. The same build copies the reviewed PWA output to
`/app/` and measures both surfaces against the one byte budget.

## VS Code implementation

The one sidebar webview owns project and conversation navigation. Selecting a row opens the provider CLI's own TUI in
an editor terminal tab. Every tab is a bounded viewer of one Runtime-owned terminal generation, so additional tabs or
windows do not create another provider process, screen model, or output ring. A custom operator name is primary when
present. Otherwise Studio uses the provider's own catalogue title when it publishes one, refreshing that metadata
after a native identity appears and after each turn settles. A project and provider fallback remains when the provider
supplies no title, and a short stable discriminator separates collisions. Stable ordering within operational bands,
fuzzy metadata search, workspace following, hot process bounds, and cold provider-native resume are release contracts,
not styling preferences.

Sidebar webview code must use the VS Code state and message boundary already covered by extension gates. It must not
receive conversation content, introduce a second transcript store, call Runtime directly, or add a provider-specific
product branch.
