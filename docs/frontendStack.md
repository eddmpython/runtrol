...
**화면도 한 번만 쓴다.** 세션 목록 · 대화 · 승인 카드 화면은 공유 패키지로 한 번 작성하고 데스크톱 (웹뷰) 과 PWA 가 같이 소비한다. 컴포넌트 (Astryx) 공유에서 한 층 더 올라간 공유이고, 두 구현이 갈라지는 것을 원천 차단한다. 데스크톱 셸이 웹뷰가 아니게 결정되면 이 층은 성립하지 않으므로, 그 결정 (`mainPlan/desktopGui/`) 이 이 절의 전제다.
