...
`crates/runtrol-drivers/src/shipped.rs` names them, one `include_str!` per manifest, and that list is the whole set.
An earlier build generated the list from a snapshot of the official ACP Registry, which put thirty unmeasured
adapters beside the three measured ones and let the sidebar advertise services the operator had never chosen. Which
services a product serves is a product decision, so the list is written by hand and grows when a service has been
measured and asked for.

Adding one stays a manifest and nothing else. The kernel selects code by `kind`, so a service that speaks a kind
this build already serves needs no new code. Runtime never contacts a package registry, never invokes `npx`, `uvx`,
an installer, or an update command, and resolves only executables already on the operator's search path.
