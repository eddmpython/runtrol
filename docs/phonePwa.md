...
The application is served from `/runtrol/app/` on the permanent GitHub Pages origin. The supported connection path is the ciphertext relay. Content-free Web Push wakes the installed application when Core publishes an event that needs attention. After its authenticated reconnect, the phone derives an exact current session destination from the bounded structural session catalogue. Direct LAN and peer-to-peer routes are not part of this release.
...
- The narrow 390 by 844 PWA session list and focused terminal states are rendered through the pinned public browser
  control and inspected directly.
